...
public class SalesResource {
  ...
  /**
   * {@code GET  /sales/:state} : get the sales with specific state.
   * @param state the state of the sales to retrieve
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of sales in body.
   */
  @GetMapping("/sales/state/{state}")
  public List<Sales> getSalesByState(@PathVariable String state) {
    log.debug("REST request to get Sales: {}", state);
    return salesRepository.findAll().stream().filter(sale -> sale.getState().toString().equals(state)).collect(Collectors.toList());
  }
